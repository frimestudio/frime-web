...
const path = "/on/repigmentacja";

export async function generateMetadata({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({
    locale,
    namespace: "services_content.repigmentacja",
  });
  const titleByLocale: Record<string, string> = {
    pl: "Repigmentacja brody i głowy w Warszawie · FRIME Wilcza 26",
    uk: "Репігментація бороди і голови у Варшаві · FRIME Wilcza 26",
    en: "Beard and head repigmentation in Warsaw · FRIME Wilcza 26",
  };
  const descByLocale: Record<string, string> = {
    pl: "Repigmentacja brody, SMP, kamuflaż blizn po przeszczepie w studio FRIME na Wilczej 26 w Warszawie. Zabieg od 80 zł, konsultacja gratis.",
    uk: "Репігментація бороди, SMP, камуфляж шрамів після пересадки у студії FRIME на Wilczej 26 у Варшаві. Процедура від 80 zł, консультація безкоштовна.",
    en: "Beard repigmentation, SMP, transplant scar coverage at FRIME studio on Wilcza 26, Warsaw. Treatment from 80 PLN, free consultation.",
  };
  return {
...
export default async function RepigmentacjaPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("services_content.repigmentacja");
    ...
    <>
      <JsonLd
        data={serviceSchema({
          name: t("title"),
          description: t("lede"),
          category: "Beard and scalp repigmentation",
          priceMin: 80,
          priceMax: 200,
          url: fullUrl,
        })}
      ...
      <section className="border-b border-[var(--color-line)]">
        <Container className="grid gap-10 py-12 md:grid-cols-12 md:py-20">
          ...
          <div className="md:col-span-5">
            <ImagePlaceholder
              ratio="4/5"
              label="Repigmentacja · before/after"
              note="Бывшее vs после: лучше всего лицо в профиль с заметным заполнением проплешин на бороде. 4:5."
            />
          </div>
        </Container>
      </section>
      ...
      <Section tone="invert">
        <div className="grid gap-10 md:grid-cols-12">
          <div className="md:col-span-5">
            <ImagePlaceholder
              ratio="3/4"
              label="Beard repigmentation · крупный план"
              note="Макро бороды до и после: видно как пигмент заполняет проплешины. 3:4."
            />
          </div>
          <div className="md:col-span-7">
      ...
      <Section>
        <div className="grid gap-10 md:grid-cols-12">
          <div className="md:col-span-7 md:order-2">
            <ImagePlaceholder
              ratio="3/4"
              label="SMP · linia włosów / kamuflaż"
              note="Голова с короткой стрижкой, видны точки SMP. До/после. 3:4."
            />
          </div>
        ...
        <dl className="mt-12 grid gap-px bg-[var(--color-line)] md:grid-cols-3">
      ...
      <Section>
        ...
        <ul className="mt-6 grid gap-4 md:grid-cols-3">
          {related.map((r, i) => (
            <li key={i}>
              <Link
                href={r.href as "/on" | "/ona" | "/team" | "/on/mullet"}
