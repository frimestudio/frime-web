...
const path = "/ona/krotkie-fryzury";

export async function generateMetadata({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({
    locale,
    namespace: "services_content.krotkie-fryzury",
  });
  const titleByLocale: Record<string, string> = {
    pl: "Krótkie fryzury damskie · pixie, bob, buzz · FRIME Warszawa Wilcza",
    uk: "Короткі жіночі стрижки · pixie, bob, buzz · FRIME Варшава Wilcza",
    en: "Short women's cuts · pixie, bob, buzz · FRIME Warsaw Wilcza",
  };
  const descByLocale: Record<string, string> = {
    pl: "Krótkie cięcia damskie w studio FRIME na Wilczej 26 w Warszawie. Pixie, bob, buzz cut, asymetria, bowl. Cena od 150 zł, konsultacja gratis.",
    uk: "Короткі жіночі стрижки у студії FRIME на Wilczej 26 у Варшаві. Pixie, bob, buzz cut, асиметрія, bowl. Ціна від 150 zł, консультація безкоштовна.",
    en: "Short women's cuts at FRIME studio on Wilcza 26, Warsaw. Pixie, bob, buzz cut, asymmetry, bowl. Price from 150 PLN, free consultation.",
  };
  return {
...
export default async function KrotkiePage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);
  const t = await getTranslations("services_content.krotkie-fryzury");
    ...
    <>
      <JsonLd
        data={serviceSchema({
          name: t("title"),
          description: t("lede"),
          category: "Short women's haircut",
          priceMin: 150,
          priceMax: 250,
          url: fullUrl,
        })}
      ...
      <Section>
        <div className="grid gap-10 md:grid-cols-12">
          <div className="md:col-span-7 md:order-2">
            <ImagePlaceholder
              ratio="3/4"
              label="Bob · 4 warianty"
              note="Коллаж: classic, з grzywką, asymetryczny, choppy. 3:4."
            />
          </div>
